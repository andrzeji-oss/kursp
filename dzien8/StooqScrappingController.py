...
import re
from dzien7.database_app.controller.TaskManagerController import toExport
...
class StooqScrappingController:
    ...
    def insertDataIntoDB(self,i):
        self.cursor.execute("INSERT INTO stooq VALUES (default, %s,%s,%s, default)", (self.result[0][i], self.result[3][i-1], self.result[1][i]))
    ...
    def formatDateTime(self, oldDate):
        monthNameConv = {
            "sty" : "01", "lut" : "02", "mar" : "03", "kwi" : "04", "maj" : "05", "cze" : "06", "lip" : "07", "sie" : "08", "wrz" : "09", "paz" : "10", "lis" : 11, "gru" :12
        }
        oldDate = oldDate[5:] # odcięcie nazwy dnia tygodnia
        oldDate = str(oldDate).split(" ") # podział daty po spacjach
        day = oldDate[0]
        month = monthNameConv[oldDate[1].replace(",","")]
